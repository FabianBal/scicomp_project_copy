...
fn main() {
    ...
    let output_filename_raw_multiplication = format!(
        "./output/data/{}_result_times_raw_multiplication_repeat_count_{}.csv",
        chrono::Local::now().format("%Y-%m-%d_%H-%M-%S"),
        repeat_count
    );
    let output_filename_overhead = format!(
        "./output/data/{}_result_times_overhead_repeat_count_{}.csv",
        chrono::Local::now().format("%Y-%m-%d_%H-%M-%S"),
        repeat_count
    );
    let output_filename_total = format!(
        "./output/data/{}_result_times_total_repeat_count_{}.csv",
        chrono::Local::now().format("%Y-%m-%d_%H-%M-%S"),
        repeat_count
    );
    let mut file_raw_multiplication =
        File::create(&output_filename_raw_multiplication).expect("Failed to create output file");
    let mut file_overhead =
        File::create(&output_filename_overhead).expect("Failed to create output file");
    let mut file_total =
        File::create(&output_filename_total).expect("Failed to create output file");
    for index in 0..results[0].len() {
        writeln!(file_raw_multiplication, "{}", results[0][index])
            .expect("Failed to write to raw multiplication output file");
        writeln!(file_overhead, "{}", results[1][index])
            .expect("Failed to write to overhead output file");
        writeln!(file_total, "{}", results[2][index])
            .expect("Failed to write to total output file");
    }
    println!(
        "exported tables to {}, {}, {}",
        output_filename_overhead, output_filename_raw_multiplication, output_filename_total
    );
}
...
fn benchmark_matrix(
    ...
    for _ in 1..=repeat_count {
        let (_matrix, time_raw_multiply, time_total) =
            cublas::multiply(&matrix1_dense, &matrix2_dense).unwrap();
        times_cublas.push((
            time_raw_multiply,
            time_total - time_raw_multiply,
            time_total,
        ));
    }
    print!(
    ...
    for _ in 1..=repeat_count {
        let (_matrix, time_raw_multiply, time_total) =
            cusparse::multiply(&matrix1_csr, &matrix2_csr).unwrap();
        times_cusparse.push((
            time_raw_multiply,
            time_total - time_raw_multiply,
            time_total,
    ...
    for _ in 1..=repeat_count {
        ...
        (_matrix_dense, time_raw_multiply, time_total) =
            gpu::dense::multiply_for_benchmark(&matrix1_dense, &matrix2_dense, 1000 * 1000 * 1000);
        //-----------------------------------------end broken part
        times_gpu_dense.push((
            time_raw_multiply,
            time_total - time_raw_multiply,
            time_total,
        ));
    ...
    for _ in 1..=repeat_count {
        ...
        let multiply_future = async {
            let mut gpusm = gpu::GPUSparseMultiplyer::new(
                &matrix1_csr,
                &matrix2_csr,
                batch_size,
                WgpuTask::new(1000 * 1000 * 1000).await,
            )
            .await;
        ...
        tokio::runtime::Runtime::new()
            .unwrap()
            .block_on(multiply_future);
        //-----------------------------------------end broken part
        let time_total = start_total.elapsed().as_micros();
        times_gpu_sparse.push((
            time_raw_multiply,
            time_total - time_raw_multiply,
            time_total,
        ));
    ...
    let times_vec: Vec<Vec<(u128, u128, u128)>> = vec![
        times_cublas,
        times_cusparse,
        times_gpu_dense,
        times_gpu_sparse,
    ...
    let min_times: Vec<(u128, u128, u128)> = times_vec
        .into_iter()
        .map(|times| {
            times.iter().fold(
                (u128::max_value(), u128::max_value(), u128::max_value()),
                |acc, time| {
                    (
                        max(1, min(acc.0, time.0)),
                        max(1, min(acc.1, time.1)),
                        max(1, min(acc.2, time.2)),
                    )
                },
            )
        })
